...
//
// Source agent: services/workers/src/agents/triage-classifier.ts
// Output schema: ClassifierOutput (defined alongside the agent).
// Model: Haiku 4.5 by default — classification is the cheapest tier.
//
// Editing this prompt:
//   1. Make the change.
//   2. Bump `version` (semver: patch for wording, minor for behavior shift,
//      major for output schema change).
//   3. Run `bun run --filter @docket/prompts test` to recompute the hash;
//      copy the recomputed value into `hash` below. Hash drift detection
//      will tell you the exact value to use.
//   4. Update `lastEdited`.
//   5. PR review, ship.

import type { Prompt } from './index.js';

const TEMPLATE = `You are the Triage Classifier for Docket, an agentic operator for tax practices.
...
export const triageClassifier: Prompt = {
  id: 'triage-classifier',
  version: '1.0.0',
  model: 'haiku-4-5',
  template: TEMPLATE,
  // Recomputed by the loader on first getPrompt() call. If this drifts
  // from sha256(`${version}|${template}`) hex-encoded, getPrompt throws
  // with the recomputed value to copy here.
  hash: 'a3d19a8590ff863a636449cd8ab4c4ac900e93f27fcc4bd51131d2a3ce960d8a',
