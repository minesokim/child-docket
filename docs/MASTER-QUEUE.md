...
| 19 | Eval suite for prompt-bearing agents | Currently zero behavior-level tests on `discovery-agent` / `inbox-drafter` / `notice-drafter` prompts. Content-invariant tests only. Session 16 partial-close: discovery-agent eval scaffold (D18) + notice-drafter eval (D19) shipped. Inbox-drafter eval has structural cases via `eval-draft.ts` (Session 7); a prompt-injection extension to that scaffold is the remaining piece. | in-progress | Claude | None | 7/01 |
