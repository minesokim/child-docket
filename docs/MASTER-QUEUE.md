...
| 19 | Eval suite for prompt-bearing agents | Three drafters covered for prompt-injection (A15 fully closed): discovery (D18, e9a742c), notice (D19, 67bbfc2), inbox (D20, this commit). Discovery covers §6695(g) Form 8867 rule (A16). Remaining items in #19: behavior-level coverage for triage-classifier (structural only today), citation-accuracy eval (gated on knowledge layer ingestion), full F1 calibration across all agents. | in-progress | Claude | None | 7/01 |
