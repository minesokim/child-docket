...
| 19 | Eval suite for prompt-bearing agents | Currently zero behavior-level tests on `discovery-agent` / `inbox-drafter` / `notice-drafter` prompts. Content-invariant tests only. Session 16 partial-close: discovery-agent eval scaffold shipped (6 cases — 4 §6695(g) happy-paths + 1 false-positive guard + 1 prompt-injection). Notice-drafter + inbox-drafter still pending. | in-progress | Claude | None | 7/01 |
