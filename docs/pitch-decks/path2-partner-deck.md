...
> *For other AI tax tool companies integrating via Docket's public API + MCP server.*
...
- Haokun Yang, CTO
- Developer docs: docs.docket.com (when public)
- MCP server: mcp.docket.com (when public)

**Visual:** technical-architecture diagram-style background. Forest green primary. Minimal text. Developer audience.

**Speaker notes:**
> "Docket is the compliance and audit substrate for the AI tax stack. We ship a public API and an MCP server. The bet: other AI tax tools embed our compliance reasoning into their workflows. Tax software vendors integrate at scale. Docket becomes the layer of plumbing the entire AI tax stack runs through."
...
**Sub-line:**
> *Docket is the layer that closes the gap. Embed Position Framework + cited authority + audit chain via our public API or MCP server. Your customers' PTIN risk is contractually bounded.*

**Speaker notes:**
> "Three things your AI tax tool is missing. One: your buyers' PTIN carries the penalty risk. Two: your AI doesn't carry the compliance layer — every position needs cited authority and tier classification and 8275 trigger. Three: your customer's E&O carrier disclaims coverage on tech-vendor tools. You're in a coverage gap. Docket closes it."

---

## Slide 3 — What Docket exposes via API
...
| **Coverage Map query** | Submit a tax surface (form, IRC section, state). Returns Docket's coverage status (Live / Pending / Out-of-Scope) + tier + reviewer. | `GET /v1/coverage/{surface}` · MCP tool `query_coverage` |
...
- Before your AI takes an action, query whether the action falls in Docket's covered scope
- If covered, route through Docket's classify + record APIs
- If not covered, your AI proceeds with explicit "out of Docket scope, professional judgment required" disclosure
- **Result**: bounded liability for the partner; AI safely confined to Docket's covered territory.

**Use case C: Bloomberg Tax / Checkpoint / CCH AnswerConnect**
- License Docket's Position Library as embedded content
- Display Docket's tier classifications next to your traditional research content
- Your subscribers get the AI defense layer + your editorial depth
- **Result**: incumbent research products stay relevant in an AI world; Docket gets distribution at scale.
...
1. **Today (Developer tier, free)**: register at developer.docket.com (when public). Get API keys. Hit `POST /v1/positions/classify` with a test position. Confirm latency + response shape match your integration spec.
...
- **Joint marketing**: your product + "Powered by Docket Compliance" badge. Your customers see the trust signal.
...
**Headline:**
> Docket. The compliance + audit substrate for the AI tax stack.
...
- Developer docs: docs.docket.com (when public)
- MCP server: mcp.docket.com (when public)
- Repo (private, NDA-gated for serious partner conversations): github.com/minesokim/child-docket
...
### Objection 3: "What if Docket gets acquired? My integration becomes a liability."
...
- **API documentation URL** (`docs.docket.com`) needs to be live before any partner conversation. If it's not live, share the markdown source under NDA.
- **MCP server URL** (`mcp.docket.com`) status: track in `docs/STATE.md`. Update slide deck when public.
