...
| C6 PostgresRetriever (BM25 + cosine + score fusion) | ⬜ Queued (next) | Implements `KnowledgeRetriever` interface in `packages/tax-graph`. Consumes `searchAuthorities()` + `lookupAuthorityByCitation()` (both already gated on reviewStatus by default per C5, so the retriever inherits the safety floor without extra wiring). |
