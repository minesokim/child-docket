...
| **Anthropic** | 🟡 **Connected but credit balance exhausted as of 2026-05-12 afternoon** | env var (`ANTHROPIC_API_KEY`) | Direct API + ZDR. Prompt caching wired into runDocketAgent. **Current blocker**: API returns 400 INVALID_REQUEST_ERROR "Your credit balance is too low to access the Anthropic API." Affects: triage-classifier + inbox-drafter agents in /e2e (3 of 7 checks fail), discovery-agent (when wired), any future feat|fix that crosses the /e2e cadence threshold. **Resolution**: top up at console.anthropic.com/settings/billing. Once topped up, re-run `pnpm --filter @docket/workers e2e` to clear the protocol-gate cadence and unblock C6 commit. Bedrock fallback exists (`callClaudeWithFallover`) but is not the default and not configured for these tests. |
...
| **C6 PostgresRetriever** | 🟡 **Code complete, BLOCKED on Anthropic credits for /e2e** (uncommitted on disk 2026-05-12 afternoon) | `packages/db/src/postgres-retriever.ts` + smoke at `packages/db/scripts/smoke-postgres-retriever.ts`. Hybrid BM25 (via existing `searchAuthorities()`) + cosine (via pgvector `<=>` against `authority_chunks.embedding` + Voyage `voyage-3-large` query embed with `input_type=query`) + RRF fusion (k=60, default over-fetch 3×topK). Inherits the reviewStatus gate from C5 (default-denies DRAFT-DAVID firm_memo; `includeDrafts: true` opt-in for dev/admin). Graceful BM25-only fallback when Voyage is unreachable / 429-exhausted / missing key (console.warn on first occurrence per retriever lifetime). Smoke 6/6 PASS: default filters drafts, includeDrafts surfaces p001+others, semantic-only "pass-through deduction" surfaces p012 CA PTET that BM25 missed (cosine arm validated), BM25-only fallback works, empty/short query → [], topK=0 → []. /e2e cadence gate BLOCKED because triage-classifier + inbox-drafter fail 400 INVALID_REQUEST_ERROR ("Your credit balance is too low to access the Anthropic API") — unrelated to C6 code; pure account-level blocker. Path forward: top up Anthropic credits → re-run /e2e → codex review → commit + push. |
| C7-C12 Discovery agent + PDF + Resend delivery | ⬜ Queued (blocked behind C6 + Anthropic credits) | Target 6/8 per Phase 3 production coding plan. Note: Discovery agent itself requires Anthropic API for prompt → tax-position generation, so C7+ is also gated on the credit top-up regardless of C6's commit state. |
