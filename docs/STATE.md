...
| **C10 Resend email delivery** | ✅ Shipped ae6a3f0 (2026-05-12 evening) | New package `@docket/email` (~320 LOC). `sendDiscoveryScanEmail(opts)` public API wraps Resend SDK with branded editorial-warm HTML body, signed-URL CTA, optional PDF attachment (belt-and-suspenders for firewall stripping), and a zero-position branch (subject "Tax Year N (already tight)" + body "We did not surface any defensible deductions..." per DISCOVERY-SCAN-OPERATIONAL.md "if the return is already tight, that's the answer"). All tenant/contact fields HTML-escaped via `escapeHtml()` covering all 5 HTML-significant chars. Expiry formatted via `Intl.DateTimeFormat` in America/New_York. Fail-fast on missing `RESEND_API_KEY` or `RESEND_FROM_ADDRESS` (no hardcoded sender — Resend rejects unverified domains). Runtime deps include `react` + `react-dom` (Resend transitively requires) for self-contained pruned installs. 7 codex rounds (9 findings fixed: conditional attachment copy, Intl expiry, HTML injection, @types/react, fail-fast from-address, docs sync, React peer deps, zero-position branch, +1 false-positive R7 from sandbox Windows ACL issue). Operator setup required: RESEND_API_KEY + RESEND_FROM_ADDRESS env vars + verify sending domain on resend.com. |
| **fix(scripts) protocol-gate path** | ✅ Shipped c61edc9 (2026-05-12 evening) | Resolved E2E_STATE_FILE relative to `git rev-parse --show-toplevel` instead of process cwd. The `/e2e` script runs from services/workers/ via `pnpm --filter @docket/workers e2e` and was writing the SHA to services/workers/.gstack/last-e2e-sha; the commit-msg hook reads from repo-root .gstack/last-e2e-sha. Caused false-block on C10 commit ("9 feat\|fix commits since ab46c057" despite multiple intervening /e2e runs that landed in the wrong location). Codex CLEAN PASS. |
| **C11 discovery_scans audit-trail** | ✅ Shipped b649bb7 (2026-05-12 evening) | Migration 0029 + Drizzle schema + apply-29 script. One row per Discovery Scan delivered tracking the rendered→delivered→opened→downloaded lifecycle. Distinct from the `actions` cryptographic audit chain (linked via `actions_row_id` composite FK). 6-value status enum. RLS-scoped to tenant + bypass policy for Resend webhooks (`app.bypass_rls = 'on'` pattern from payments/gmail_sync_state). Composite FKs to clients + actions on (tenant_id, *) enforce same-tenant references; column-specific `ON DELETE SET NULL (client_id)` and `SET NULL (actions_row_id)` because tenant_id is NOT NULL. Cross-column CHECK: highest_tier NULL when positions_count=0 (zero-position scans are valid Discovery outcomes), 1-4 otherwise. UNIQUE partial index on email_id (non-NULL) for unambiguous webhook lookup. Child-side partial indexes on (tenant_id, client_id) + (tenant_id, actions_row_id) so parent-delete pruning doesn't full-scan. 7 codex rounds (9 substantive findings fixed: RLS webhook bypass, composite FK tenant enforcement, impossible bare SET NULL on NOT NULL composite, Drizzle/SQL drift on delete semantics, zero-position support, email_id uniqueness, named CHECK constraint, index direction, child-side FK indexes). R7 CLEAN PASS. Bootstrap chain updated. Apply-29 smoke 8/8 PASS. |
| **C12 /scan landing + prospects table** | ✅ Shipped 3df7882 (2026-05-12 night) | `apps/client-portal/src/app/scan/page.tsx` + `scan-landing-client.tsx` (~1,200 LOC) — full 7-section landing page per locked copy: Hero / What you get / How it works / Pricing math (§6695(g) + §6694 numbers with IRS source) / Founders / FAQ (8-item collapsible) / Form (10 fields + redaction confirmation) / Footer. Editorial-warm tone (cream canvas + forest green primary + Fraunces + DM Sans). Form posts to `/api/scan-intake-stub` which validates + persists to the new prospects table (migration 0030, pre-tenant, no RLS). Rate-limited 10/5min per IP. Closed allowlists on source/firm_size/designation prevent PII smuggling into Sentry tags. PII discipline: Sentry breadcrumbs carry only prospect_id + non-PII enums; DB failure path uses operational tags only (no contact info). Honest copy: "follow-up within one business day" (manual review through founder-30) — C12b lifts to "upload link in 5 min" with Resend integration. `/scan(.*)` allowlisted in middleware (trailing-slash safe). 12 codex rounds (22+ findings fixed across access-control / persistence / PII / migration / copy-honesty / CI surfaces). Protocol-Skip on R12 (OpenAI codex provider at capacity). Apply-30 verified on dev; CI workflow extended to chain apply-26-27 + apply-28 + apply-29 + apply-30 so future deploys auto-apply. |
| C12b Resend confirmation + presigned upload URL | ⬜ Queued | After RESEND_API_KEY + RESEND_FROM_ADDRESS are set in env, the form-submit fires an immediate confirmation email with a presigned R2 upload URL (replaces the "follow-up in 1 day" interim copy with the original locked "secure upload link in 5 min"). Uses `@docket/email` (C10). Operator action required: complete Resend domain verification at resend.com. |
| C13-C23 (Twilio inbound + 8879 production wiring + nudges substrate + freshness view + Projects substrate, etc.) | ✅ Shipped (see `git log --oneline` between c61edc9..457708b — not yet folded into STATE.md row-by-row) | Cluster of ~11 commits 5/12 night → 5/13 covering reminders/notifications/nudges/projects substrates + freshness view + Trust Center + per-project drill-down. Each one shipped through the full /overnight ritual (codex PASS + Score ≥95 + Align ALIGNED). Granular per-commit detail lives in the commit messages themselves; this row is a placeholder so the catch-up is visible. |
| **C24 per-project drill-down /projects/[id]** | ✅ Shipped b028270 (2026-05-13 morning) | Per-project page showing template metadata + derived instances (templates) or attached engagements + stage breakdown + source-template trace (instances). Two-phase loader (cheap head → role gate → expensive query). UUID validation regex prevents 500 on malformed URLs. Stage counts from separate aggregate query, not LIMIT 500 display list. DerivedInstanceRow conditionally renders as Link or styled div based on canViewInstance. Also fixed latent C21+C23 Server Actions rule violation: extracted CANONICAL_PROJECT_TEMPLATES + CANONICAL_NUDGE_RULES + getCanonicalRuleMetadata + getCanonicalTemplateMetadata to new metadata.ts files (Next.js rejects non-async exports from 'use server' files at build time). TemplateCard + InstanceCard on /projects wrapped in Link for normal-navigation reachability. 5 codex rounds with findings fixed each round. Score 96/100. |
| **C25 Project assignment UX from /clients/[id]** | ✅ Shipped 96ebb76 (2026-05-13 evening) | Operationalizes the C23+C24 substrate — picker on /clients/[id] inside the Engagement Section, with attach/detach/primary controls. Clone-on-attach: when user picks a template, server transparently materializes a derived instance for the engagement's tax_year (or reuses an existing one for the same template+year), so the template/instance mental model stays clean and /projects + /projects/[id] views stay coherent. Migration 0035 adds a partial unique index `engagement_projects_one_primary_per_engagement_uniq_idx` enforcing one-primary-per-engagement at the DB layer, with a dedup pre-step for safe deployment. SELECT FOR UPDATE on target attachment in setPrimaryProject + split clear+set across two statements (target excluded from clear) so no transient two-primary state can trip the partial unique index. 5 codex rounds (caught: stock-tenant pool empty / cross-tenant project / primary lost on conflict / revalidate-layout vs page / stale-target race / concurrent-promotion invariant / single-stmt-trips-index / migration-deploy-risk / LIMIT-before-filter / attach-to-template-breaks-views / primary-uses-template-id / setPrimary-no-row-lock / template-resurfaces-after-instance — all fixed). Score 96/100. |
| **C26 editable per-attachment notes on /projects/[id]** | ✅ Shipped 13cc9d6 (2026-05-13 evening) | Closes the third-primitive triangle — engagement_projects.notes was loaded read-only by C24 and inline-rendered; C26 makes it editable from /projects/[id] instance view via a new server action `setEngagementProjectNotes` + lazy-loaded inline editor. Thin client wrapper renders saved text + "+ Add note" button server-side; React.lazy + Suspense defers the heavy textarea + save handlers + char-count machinery until the user clicks Edit. Without the split, every engagement row (up to 500 per LIMIT 500) would hydrate stateful textareas. 500-char server-side cap, trim+null-coerce empty/whitespace, RETURNING-based not-found detection. 2 codex rounds (r1: P2 500-row hydration cost — lazy-load fix; r2 CLEAN PASS). /e2e PASS 8/8 at c1645c15 pre-commit. Score 95/100. |
| **C27 archive + unarchive surface on /projects/[id]** | ✅ Shipped 092b705 (2026-05-13 evening) | Firm-owner-only archive primitive on instance projects (templates rejected server-side AND in UI — no in-product unarchive path for canonical templates since seedProjectTemplates uses ON CONFLICT DO NOTHING). Inline confirm-pattern button in /projects/[id] header. Archive cascade clears engagement_projects.is_primary for the archived project (codex r5 P2) so /clients/[id] doesn't render archived projects as Primary. attachEngagementToProject hardened: pre-flight is_active check + in-transaction FOR UPDATE row lock on projects.id closes the TOCTOU race. Clone-on-attach archived-match handling: when active instance doesn't exist but archived one does, throw ArchivedMatchError sentinel, surface friendly "ask firm owner to unarchive" error (auto-unarchive would be role bypass since unarchive is firm_owner-only). setPrimaryProject FOR UPDATE OF ep, p locks both rows so concurrent archive can't race past the archived-target check. 5 codex rounds with 12 findings caught + fixed (r1 revalidate-scope + reject-inactive-attach; r2 archived-blocks-reattach + templates-dead-end; r3 r2-auto-unarchive-was-role-bypass-revert + setPrimary-archived-reject; r4 archive-rejects-template-server + attach-TOCTOU-FOR-UPDATE; r5 archive-demote-primary-cascade + setPrimary-locks-projects-row). Score 95/100. |
