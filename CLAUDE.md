...
3b. **Read [`docs/ASSUMPTIONS-TO-TEST.md`](docs/ASSUMPTIONS-TO-TEST.md)** — every prior session's ASSUMED claims, captured here so they don't disappear into chat history. Scan for `open` rows whose verification path matches the current session's context (a deploy, a CI run, partner #2 onboarding, etc.) and proactively remind David. Habit codified 2026-05-16 (Session 15) per David's behavioral instruction: "note all assumptions for testing later. remind me. make this a habit."
