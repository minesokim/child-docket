...
- [`/keep-going`](.claude/skills/keep-going/SKILL.md) — runs BETWEEN items. Kills the natural-pause-handoff anti-pattern. After a clean commit + deploy + verify cycle, pick the next item from queue → followups → PRODUCTION-READINESS automatically. Do NOT pause to ask "which direction next?" Stop only when a real stop condition fires (three-strikes-stop, smoke fail, security finding, >10 file commit, 3+ pending architectural decisions, context <10% remaining, or user explicitly says pause). User-codified 5/8/2026: "i wanted it to keep going here. until the feature list is complete."

The full cycle: plan → /edge-cases → implement → typecheck → test → /code-quality (lockfile, anti-patterns, codex if substantial) → commit (with /decisions-log entry if applicable) → push → verify deploy READY (curl test endpoint if applicable) → /smoke-test if applicable → /keep-going (pick next item) ⟲
